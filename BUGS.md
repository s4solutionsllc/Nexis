...
- [ ] **BUG-109: APT Repo Manager "Available Updates" indicator does not recenter/realign on window resize** (LOW)
  - **File:** `shared/nexis/Pages/AptSourceManager/apt_source_manager_page.cpp`
  - **Description:** The "Available Updates" indicator on the APT Source Manager page does not recenter or realign when the application window is resized. It only renders correctly at its initial geometry; after resizing the window, the indicator's position/alignment becomes visibly off until the user forces another layout pass. Likely a missing `resizeEvent()` reposition call (similar pattern to BUG-103 / BUG-78). Fix: add a `resizeEvent()` override (or a layout-based repositioning) that re-centers and realigns the updates indicator whenever the page is resized.
