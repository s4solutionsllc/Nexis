...
- [x] **BUG-134 / #21**: [Bug] Cramped UI since 2.2.13 (MEDIUM)
  - **Platform:** Linux (Kubuntu 26.04, 1366×768)
  - **Description:** UI layout became cramped starting in v2.2.13. Maximizing the window does not improve the situation, suggesting fixed minimum sizes or non-responsive layout constraints were introduced. Reported on 1366×768 resolution.
  - **Resolved:** Sidebar nav sections wrapped in QScrollArea between pinned logo and pinned footer. Thin scrollbar appears only on overflow. Badge viewport clip check added to `repositionBadges()`. See commit for details.
...
- [x] **BUG-102: .deb packages report wrong version — binary shows v2.1.16 instead of v2.1.18** (HIGH)
  - **Files:** `.github/workflows/release.yml`, `CMakeLists.txt`, `linux/debian/rules`
  - **Description:** The release workflow syncs debian/changelog to the git tag version (e.g., 2.1.18), so `dpkg -l` shows the correct version. However, the binary is compiled *before* any version sync, using `APP_VERSION` from CMakeLists.txt which was never bumped. The compiled-in version (shown in About/title bar) stays at 2.1.16 while the .deb metadata says 2.1.18.
  - **Resolved:** Added `APP_VERSION_OVERRIDE` cmake variable. When set, overrides `PROJECT_VERSION` for the compiled-in `APP_VERSION`. All three release jobs (Noble .deb, Plucky .deb, macOS .dmg) now pass the git tag version to cmake at configure time. `debian/rules` conditionally forwards the env var. Local builds without the override are unchanged.

- [x] **BUG-103: System Cleaner schedule indicator renders as white square on initial page load** (MEDIUM)
  - **Scope:** System Cleaner page — schedule indicator widget
  - **Description:** When navigating to the System Cleaner page for the first time, the schedule indicator at the top of the page renders as a small white/blank rectangle instead of showing the next/last schedule text. Resizing the window causes Qt to re-layout and the indicator renders correctly at the bottom of the page with proper styling. Root cause: `repositionScheduleIndicator()` was called during the constructor when `width()`/`height()` returned default values, placing the indicator at the wrong position. No `showEvent()` override existed to reposition once valid geometry was available.
  - **Files:** `shared/nexis/Pages/SystemCleaner/system_cleaner_page.cpp`, `shared/nexis/Pages/SystemCleaner/system_cleaner_page.h`
  - **Resolved:** Added `showEvent()` override that calls `repositionScheduleIndicator()` and `repositionExclusionsButton()`, matching the existing `resizeEvent()` pattern.
...
- [x] **BUG-137: Dashboard Memory (MetricTile/sparkline) tile shows white card background instead of transparent** (LOW)
  - **Files:** `shared/nexis/Pages/Dashboard/metric_tile.cpp`
  - **Description:** `MetricTile` was the only tile type with `setAttribute(Qt::WA_StyledBackground, true)`, causing the QSS `background-color: @cardBg` rule to paint a solid card behind the Memory tile. All other tile types (RingTile, HybridTile, GaugeTile, etc.) lack this attribute and appear transparent. Additionally, `QChartView`'s viewport auto-filled with a dark/black color before the chart rendered, and the chart background was set to fully transparent rather than `@cardBg` (inconsistent with NetworkTile's sparkline treatment).
  - **Resolved:** Removed `WA_StyledBackground` from `MetricTile` constructor; added `mChartView->viewport()->setAutoFillBackground(false)` to prevent black viewport pre-fill; restored `@cardBg` chart background brush to match NetworkTile's sparkline container pattern. PR #29.
