...
### Fixed
- **macOS ARM64 build broken by BTM header platform guard (SSO-3765, regression from SSO-3738 / FW-10):** `btm_row.h` and `btm_reset_dialog.h` placed `#ifdef Q_OS_MACOS` before any Qt include, so the platform macro (defined by `<QtGlobal>`) was not yet visible. The first translation unit to scan each header — the .cpp itself and AUTOMOC — saw an empty class declaration, so the .cpp.o ended up with no `BtmRow` / `BtmResetDialog` symbols and AUTOMOC emitted "No relevant classes found" for both headers. On Apple builds the linker then failed `nexis.app` and `test-CleanerExclusionTests` with `Undefined symbols for architecture arm64: BtmRow::staticMetaObject`, `BtmRow::BtmRow(BtmRecord const&, QWidget*)`, `BtmRow::matches(QString const&) const`, and `BtmResetDialog::BtmResetDialog(QWidget*)`, all referenced from `libnexis-gui.a[startup_apps_page.cpp.o]`. Fixed by moving the existing `<QWidget>` / `<QDialog>` include above the platform guard in each header so `Q_OS_MACOS` is defined when the guard is evaluated. Linux builds were unaffected because the .cpp files are only added to `GUI_PLAT_SRCS` on `APPLE`.
