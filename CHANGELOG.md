...
### Added
- **macOS deep-maintenance panel (FW-17):** Added a new "macOS Maintenance" modal dialog (`MacOSMaintenancePanel`) accessible via the Health Score tile quick action on macOS. Exposes 8 OnyX-style tasks — Rebuild Spotlight Index (`mdutil -E /`), Verify Disk (`diskutil verifyVolume /`), Rebuild Launch Services (`lsregister` + Finder restart), Flush DNS Cache (`dscacheutil -flushcache` + `killall -HUP mDNSResponder`), Show Hidden Files in Finder, Show Path Bar, Show Status Bar, and Add "Quit Finder" menu item (the latter four via `defaults write com.apple.finder … -bool true` + Finder restart). Each task shows a description, asks for confirmation, runs asynchronously via `QtConcurrent::run()` using the WI-01-safe QPointer/destructor-waitForFinished worker-lifetime pattern, and surfaces inline ✓/✗ status with truncated output. The `HealthScoreTile::setQuickAction` no-op is now implemented so the footer button is visible. `MacOSMaintenanceTests` (8 slots, `tests/core/test_macos_maintenance.cpp`, `if(APPLE)` in CMake) verify argv construction for every task via the `defaultTasks()` static seam.
