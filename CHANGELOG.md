...
- **Rootkit Scanner frontend (FR-122, Linux):** New Helpers-page card (nav button hidden unless `chkrootkit` or `rkhunter` is installed). Launches the scanner via `pkexec` and streams stdout/stderr live to a scrollable output view. Post-scan summary applies `@successColor`/`@warningColor` to a one-line result label — "INFECTED" detection for chkrootkit, warning-count for rkhunter.
