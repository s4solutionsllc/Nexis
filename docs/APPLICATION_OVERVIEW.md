...
Dashboard, Hardware Info, Resources, Network Usage, System Cleaner, Disk Tools, Search, Processes, Services, Startup Apps, Boot Analysis, Uninstaller (Applications on macOS), Mail Cleanup (macOS), File Shredder, Helpers, System Logs, Settings — in sidebar order. Disk Map (MONITOR) and Feedback (footer) open dialogs rather than pages.
...
Continuous per-interface network data usage tracker with monthly cap management. Located in the MONITOR sidebar section after Resources and the Disk Map button.
