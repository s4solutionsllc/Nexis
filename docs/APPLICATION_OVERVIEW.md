...
- **Maintenance Wizard** (FR-83) — "System Checkup" modal dialog. Runs 4 checks in parallel via `QtConcurrent::run()`: (1) cleanable junk scan, (2) orphan package detection, (3) pending system updates, (4) health score calculation. Each check shows a running/complete/warning/error icon with detailed results. "Clean Safe Items" button auto-cleans low-risk categories (package cache, crash reports, app logs/caches, dev tool caches, broken symlinks, snap/flatpak revisions on Linux). Theme-aware with step status colors from `@successColor`/`@warningColor`/`@destructiveColor` tokens.
- **macOS Maintenance Panel** (FW-17, macOS only) — OnyX-style deep-maintenance dialog launched from the Health Score tile quick action on macOS (`MacOSMaintenancePanel`). Exposes 8 clearly-labeled tasks, each with a description and confirmation before execution: **Rebuild Spotlight Index** (`mdutil -E /`), **Verify Disk** (`diskutil verifyVolume /`), **Rebuild Launch Services** (`lsregister -r -domain local -domain system -domain user` + `killall Finder`), **Flush DNS Cache** (`dscacheutil -flushcache` + `killall -HUP mDNSResponder`), and four Finder hidden-defaults toggles (**Show Hidden Files**, **Show Path Bar**, **Show Status Bar**, **Add "Quit Finder" Menu Item** — each via `defaults write com.apple.finder …` + `killall Finder`). Tasks run asynchronously via `QtConcurrent::run()` with the WI-01-safe QPointer + destructor-waitForFinished worker-lifetime pattern. Inline status labels show ✓/✗ and truncated output on completion.
