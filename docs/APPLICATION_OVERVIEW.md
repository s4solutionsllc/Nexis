...
- **Treemap depth redesign (SSO-24963):** the Treemap mode shows two folder levels at once as nested frames — parent folders render as labelled frame groups with their children's tiles nested inside — built by the `TreemapLayout` helper (`treemap_layout.h/.cpp`, a pure squarify-layout helper with no `QWidget` dependency). Tiles light up and lift on hover. Drilling in or out plays a 200ms `OutCubic` zoom transition between the old and new layouts, skipped in favor of an instant cut when `Utilities::prefersReducedMotion()` reports the system Reduce Motion setting is on (macOS).
