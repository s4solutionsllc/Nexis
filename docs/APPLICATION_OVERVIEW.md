...
- **Storage** — Per-drive: name, size, model, SMART health verdict (Good/Caution/Critical/Unknown), color-coded. On Linux, if any drive requires elevated permissions for SMART data, two buttons appear at the top of the section: **Unlock All Drives** (runs `pkexec smartctl` for all locked drives this session) and **Make Permanent** (runs `pkexec setcap cap_sys_rawio+ep $(which smartctl)` to grant the capability permanently — buttons disappear after success). Per-drive Unlock buttons also remain for granular control.
