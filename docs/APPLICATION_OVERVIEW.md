...
| Metric | Value | Source of truth |
|--------|-------|-----------------|
| Always-visible pages | 17 | `shared/nexis/Pages/` (Dashboard, HardwareInfo, StartupApps, BootAnalysis, SystemCleaner, DiskTools, DiskMap, Search, Services, Processes, Uninstaller, Shredder, Resources, Network, Helpers, SystemLogs, Settings) |
...
| Tool classes | 10 | `shared/nexis-core/Tools/` — 6 wired through `ToolManager` (`ServiceTool`, `PackageTool`, `AptSourceTool`, `GnomeSettingsTool`, `RepoHealthChecker`, `RepoRepairEngine`) plus `DockerTool`, `FileSearchTool`, `MacDefaultsTool`, and `MacTweaksCatalog` (SSO-23857) consumed directly by their services/widgets |
| Utility classes | 6 | `CommandUtil`, `DisplayServerUtil`, `FileUtil`, `FormatUtil`, `HeadlessUtil`, `MacOsVersionUtil` (SSO-23857) in `shared/nexis-core/Utils/` |
...
Dashboard, Resources, Processes, Network Usage (MONITOR) · Hardware Info, Boot Analysis, System Logs (DIAGNOSE) · System Cleaner, Disk Tools, Disk Map, Mail Cleanup (macOS), File Shredder, File Search (CLEAN) · Uninstaller (Applications on macOS), Startup Apps, Services, APT Repository Manager / Homebrew, Docker (MANAGE) · Helpers, GNOME Settings (TOOLS) · Settings (pinned footer) — in sidebar order. Feedback (footer) opens a dialog rather than a page.
...
- **Built-in Treemap / Disk Map page (FW-09, SSO-3737; hosted as a page SSO-24963):** secondary "Built-in Treemap" button on the same card navigates to the **Disk Map page** (`DiskMapPage`, `Pages/DiskMap/`) by emitting `sigNavigateToPage("diskMap")` — the same page is also reachable from the sidebar's CLEAN-group **Disk Map** button (see [Navigation](#navigation)), the tray quick menu, and the Command Palette. The page runs a `DirSizeScanner` on a `QtConcurrent` worker, then renders the scan (pure `QPainter`) via one of three interchangeable visualization modes. Supports drill-down (double-click), hover tooltips, "Reveal in file manager", and "Move to trash" (reuses `FileSearchService` → cleaner trash path). Skips symlinks and dedups hard links so byte counts match what Baobab/DaisyDisk would report. A scan keeps running if you navigate away to another page and back. The external-tool launcher remains as a parallel option.
- **Visualization picker (SSO-23862):** a toolbar picker switches live between **Treemap** (squarified, `TreemapView`), **Bubble Map** (circle-packing, `BubbleMapView`), and **Sunburst** (radial, `SunburstView`) without re-scanning — all three share the same `DirSizeNode` scan data and drill-down/hover/context-menu behavior via a common `DiskMapView` base class, and a themed `@diskMapHue1`–`@diskMapHue8` palette with contrast-aware tile labels.
- **Two-level depth redesign, all three modes (SSO-24963):** every mode now shows two folder levels at once, each built by its own pure geometry helper (no `QWidget` dependency) so layout and painting stay separate. **Treemap** (`treemap_layout.h/.cpp`) frames parent folders as labelled nested frames with their children's tiles inside. **Bubble Map** (`bubble_layout.h/.cpp`) circle-packs a parent folder into a labelled membrane with its children's bubbles packed inside. **Sunburst** (`sunburst_layout.h/.cpp`) lays ring 0 out as wedges swept proportional to size, then subdivides each ring-0 directory's arc with a ring-1 wedge per child; a ring-0 file or empty directory gets a single desaturated ring-1 placeholder wedge spanning the same arc and resolving to the same node, so hover/drill/tooltip never dead-end. Very small siblings fold into one muted remainder wedge that resolves to their parent folder rather than each getting an imperceptible sliver, but at least the 12 largest siblings are always shown as real wedges first. Every mode lights its shapes (soft shadow, banded/radial gradient highlight) and lifts the hovered one a few pixels with a contrast outline. Drilling in or out either plays TreemapView's own 200ms `OutCubic` zoom transition or, for BubbleMapView and SunburstView, a shared cross-fade owned by `DiskMapView` (`armCrossFade()`/`startCrossFadeIfArmed()`/`paintCrossFadeOverlay()`) between a snapshot of the outgoing layout and the new one — both skipped in favor of an instant cut when `Utilities::prefersReducedMotion()` reports the system Reduce Motion setting is on (macOS).

### 10a. Network Usage

Continuous per-interface network data usage tracker with monthly cap management. Located in the MONITOR sidebar section after Processes.
...
**Tweaks (macOS only, SSO-23857, part of the SSO-15367 Power Toolkit epic)** — Searchable pane of hidden `defaults` preferences across five categories: Finder, Dock, Screenshots, Animations, and Login Window (15 tweaks total). Each row shows the tweak's live current value (`defaults read`, falling back to a documented system default when unset), an inline control (checkbox/combo/spinner/text depending on type), and a "Revert" action (`defaults delete`, verified by re-reading afterward). Writes apply immediately via `defaults write` plus the relevant `killall` (Finder/Dock/SystemUIServer); login-window tweaks write to the system domain via `sudoExecWithStatus`. A search box filters by name/category/description in real time. Tweaks with a minimum macOS version (`MacTweaksCatalog::isSupported()`) are disabled with an explanation instead of silently no-oping. Bool tweaks get a "Toggle" command palette entry; other tweaks get "Reset to Default" — both invocable without opening the pane (`App::setupCommandPalette()`). Backend split for testability: `MacDefaultsTool` (`shared/nexis-core/Tools/mac_defaults_tool.*` — pure argv-build/output-parse + exec glue), `MacTweaksCatalog` (`mac_tweaks_catalog.*` — the tweak definitions plus toggle/reset orchestration), and `MacOsVersionUtil` (`shared/nexis-core/Utils/mac_os_version_util.*` — OS-version gating). Widget: `MacTweaksWidget` (`mac_tweaks_widget.*`), following the same lazy-loaded Helper-widget pattern as Swappiness/TRIM/Battery Threshold.
...
2. Register a lazy page slot for every always-visible page (and the three conditional pages when their tools are detected); each page widget is constructed on first navigation, behind a "Loading …" placeholder, and the Dashboard is built eagerly
...
The sidebar is **collapsible**, organized into five labelled groups — **MONITOR**, **DIAGNOSE**, **CLEAN**, **MANAGE**, and **TOOLS** — matching the logical grouping of the pages. DIAGNOSE, CLEAN, MANAGE and TOOLS have a **clickable section header** (MONITOR is headerless and always expanded) with a chevron indicator that toggles the group's visibility. The CLEAN group also hosts the **Disk Map** page button (SSO-23863), sitting after Disk Tools: it is a normal checkable page button switching to the `DiskMapPage` stacked page (like any other sidebar entry), giving the built-in treemap visualizer (previously reachable only via the "Built-in Treemap" button inside the Resources page's Disk Usage Launcher card) top-level discoverability. It is also available as a "Disk Map" command in the Command Palette. Collapsed groups hide their child buttons with a smooth 200ms height animation; the collapsed/expanded state is persisted per-section across sessions as JSON in QSettings, keyed on stable section ids (a group split out by a regrouping inherits the state of the group it came from). Navigating to a page in a collapsed group (via tray menu, command palette, or `sigNavigateToPage`) auto-expands that group. The full sidebar can also collapse to a 64 px icon-rail showing only page icons separated by group divider lines (every group is shown in rail mode, even one collapsed in the expanded sidebar); the collapse and expand transitions use a smooth width animation. The sidebar can be toggled with the **Ctrl+B** keyboard shortcut or the collapse button at the top of the panel. The sidebar header displays a **gradient logo** (full wordmark when expanded, lettermark when collapsed) above a **separator line**, with a **version label** below. Active page badges use a cleaner dot indicator in collapsed mode and are hidden when their section is collapsed. The Homebrew/APT button displays an **updates badge** showing the pending update count when the sidebar is expanded, or a colored dot (using `@updatesColor`) when collapsed.

A **Command Palette** (activated with **Ctrl+K**, ⌘K on macOS) provides a substring-search popup for navigating directly to any page and executing common actions (e.g., "Run System Cleaner Scan", "Kiosk Mode", "Feedback", "Mini Monitor"); a single click or Enter runs the selected command. The same destinations are exposed through the menu bar (Settings ⌘, · View · Go with Ctrl/⌘+1–9 · Help), which is native on macOS and hidden-but-active on Linux without touching the sidebar.

Sidebar buttons trigger `SlidingStackedWidget::slideInIdx()` with a 200 ms OutCubic vertical slide whose direction follows sidebar order; a navigation that arrives mid-slide settles the running animation first so the shown page, sidebar highlight and page lifecycle never diverge. **Settings** is pinned in the sidebar footer with Feedback, and the window title names the current page. F11, the tray action, and the Dashboard button all toggle kiosk mode through synchronized signals.
