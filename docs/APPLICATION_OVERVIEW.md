...
Dashboard, Resources, Processes, Network Usage (MONITOR) · Hardware Info, Boot Analysis, System Logs (DIAGNOSE) · System Cleaner, Disk Tools, Mail Cleanup (macOS), File Shredder, File Search (CLEAN) · Uninstaller (Applications on macOS), Startup Apps, Services, APT Repository Manager / Homebrew, Docker (MANAGE) · Helpers, GNOME Settings (TOOLS) · Settings (pinned footer) — in sidebar order. Disk Map (CLEAN) and Feedback (footer) open dialogs rather than pages.
