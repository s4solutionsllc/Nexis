...
- **Network** — `NetworkTile` with two-row layout: Download and Upload labels each paired with a separate `QChart` sparkline instance (dual RX/TX charts), horizontal divider, and active interface name (1s refresh). Bind to a specific network interface via the gear menu to monitor that NIC's throughput (GH#191).
