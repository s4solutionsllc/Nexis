...
> Last updated: 2026-06-21 | Version 2.6.0
...
> **Linux minimum distribution for the `.deb` package: Ubuntu 25.04 (Plucky) / Debian 13+ (GH#185 / SSO-7304).** The Qt floor is **Qt 6.8 LTS**; Ubuntu 22.04 (Qt 6.2) and 24.04 (Qt 6.4) do not ship a satisfying `libqt6charts6`, so the `.deb` is not installable on those releases. Linux Mint 21.x (Ubuntu 22.04 base) and Linux Mint 22 (Ubuntu 24.04 base) are likewise unsupported for the `.deb`. **Users on Ubuntu 22.04 / 24.04 or any current Linux Mint release should use the AppImage**, which bundles Qt and runs on any Linux with glibc ≥ 2.35.
