...
> Last updated: 2026-07-20 (SSO-15379, SSO-15374, SSO-3469 / WI-05.a, SSO-3497, SSO-3383, SSO-3391 / WI-29, SSO-3396 / WI-33, SSO-3738 / FW-10, SSO-3739 / FW-11, SSO-3740 / FW-12, SSO-3743 / FW-15, GH#191, GH#182, GH#214) | Version 2.8.3
...
- **Per-process network on Linux — eBPF preferred, nethogs fallback (SSO-15379)** — closes the gap FR-59 left on Linux (rates stayed at `-1.0`/"—" forever). `NetAcctBpfLoader` (`linux/nexis-core/Info/net_acct_bpf_loader.{h,cpp}`) loads a small CO-RE-free BPF object (`linux/nexis-core/Info/ebpf/net_acct.bpf.c`) with two kprobes — `tcp_sendmsg` (tx) and `tcp_cleanup_rbuf` (rx) — aggregating bytes into a pid-keyed `BPF_MAP_TYPE_HASH`; neither probe reads kernel struct fields (only scalar args), so no `vmlinux.h`/BTF struct-relocation dependency, just plain kprobe attach. `ProcessInfoLinux::collectProcesses()` calls `bpf_map_lookup_elem` per PID and delta-tracks the cumulative counters exactly like the FR-108 disk-I/O block (`mPrevNetIo` + `mNetTimer`, same shape as `mPrevDiskIo`/`mIoTimer`). The BPF object is compiled by a CMake custom command (`clang -target bpf`) only when `pkg_check_modules(libbpf)` and a `clang` binary are both found; the `NEXIS_HAVE_EBPF` compile definition it gates is `PUBLIC` on `nexis-core` (not `PRIVATE`) because it changes `NetAcctBpfLoader`'s member layout — any other target including that header, e.g. the test suite, must see the same macro state or the two translation units disagree on the class's size/offsets. When eBPF can't load (no CAP_BPF/root, unsupported kernel, or built without libbpf), `ProcessInfoLinux` falls back to `NetHogsStreamer` — a persistent `nethogs -t` child in the exact `NettopStreamer` shape (QProcess + `readyReadStandardOutput` + a pure static line parser exposed for fixture tests) — except nethogs' trace-mode output is already a live rate (KB/s), not a cumulative counter, so that path skips delta-tracking and applies it directly. Collection status (`Disabled`/`ActiveEbpf`/`ActiveNetHogs`/`ActiveNetTop`/`PermissionDenied`/`Unavailable`) lives on the `ProcessInfo` base class (`netIoStatus()`/`netIoStatusDetail()`) so `ProcessesPage::updateNetIoNotice()` can show a DS §5 `[status="warning"]` notice above the table instead of leaving Net Down/Net Up silently blank when collection is on but not actually producing data — the pre-existing per-row `—` (for `-1.0`) still means "no data for this row" and is unchanged. Scope: TCP only (matches BCC's `tcptop`, the reference this mirrors); UDP would need a different uprobe pair. Environment note: this sandbox has neither CAP_BPF/root nor a guaranteed BTF/libbpf-enabled build toolchain, so the eBPF load/attach path itself was not runtime-verified here — CI's Linux job now installs `libbpf-dev`/`clang`/`libelf-dev` specifically so the BPF program at least compiles for real (see `.github/workflows/build.yml`); see `docs/plans/2026-07-20-sso-15379-process-network-ebpf-design.md` for the full verification matrix.
