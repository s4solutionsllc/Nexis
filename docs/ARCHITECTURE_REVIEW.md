...
- **Streaming subprocess pattern** — `NettopStreamer` (FR-102, macOS-only) is the first long-lived `QProcess` in the codebase using `readyReadStandardOutput` — a single `nettop -P -d -s1` child streams per-process network deltas into a mutex-guarded `QHash` that `ProcessInfoMacOS::updateProcesses` reads from. Lifecycle is tied to the `ProcessInfo::mCollectNetIO` toggle (FR-108) so the child only runs while network columns are visible on the Processes page. `NvidiaSmiCache` (FR-106, Linux-only) is a lighter variant — no persistent child, but one batched `nvidia-smi` call per tick, results shared between `GpuInfoLinux` and `FanInfoLinux`.
