...
The runtime checks (1, 2) emit `qWarning()` at runtime (visible in debug output) without altering application behavior; check (3) is a CTest assertion that fails CI on regression. Together they catch typos, missing tokens, malformed color values, and raw `@token` literals smuggled into inline stylesheets.
