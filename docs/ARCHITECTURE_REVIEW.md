...
> Last updated: 2026-09-22 (SSO-23855, SSO-23896, SSO-3469 / WI-05.a, SSO-3497, SSO-3383, SSO-3391 / WI-29, SSO-3396 / WI-33, SSO-3738 / FW-10, SSO-3739 / FW-11, SSO-3740 / FW-12, SSO-3743 / FW-15, GH#191, GH#182, GH#214, SSO-15566, SSO-17776, SSO-21680, BUG-56, SSO-23861, SSO-23862, SSO-23866, SSO-23867, SSO-23853, SSO-23854) | Version 2.12.0
...
- **Cache Rebuild widget (SSO-23866, macOS-only, part of the SSO-15367 Power Toolkit epic)** — `CacheRebuildWidget` extends the Helpers-page card pattern to a *sequence* of commands per action instead of a single write: `commandsFor(Action)` returns an ordered `QList<Step>` (cmd/args/needsSudo/description), executed one at a time on a `QThreadPool` worker via `sudoExecWithStatus`/`execWithStatus`, with a `QMetaObject::invokeMethod` back to the UI thread before each step to update a "Step N of M" progress label — addressing the epic's opaque-progress complaint about OnyX-style tools. `commandsFor()` and the pure version-gate `supportInfo(Action, QOperatingSystemVersion)` are static and platform-independent so `CacheRebuildWidgetTests` exercises command construction and the Big-Sur/Yosemite availability cutoffs without a live macOS rebuild; the widget additionally does a runtime `CommandUtil::isExecutable()` check before enabling a button, in case a supported OS is missing the underlying tool.
...
**Completed (Phase 7, FR-36; expanded FR-76, FR-79/FR-80, FR-82, FR-66, FR-68).** 23 test executables with ~312 test methods:
...
5. **Service logic** — HostService (25), DuplicateFinder (19), DiskTreemapDeleteConfirm (2)
...
| `shared/nexis/Pages/DiskMap/bubble_map_view.h/.cpp`, `bubble_layout.h/.cpp` | SSO-23862, two-level redesign in SSO-24963: bubble-map (circle-packing) mode. `BubbleLayout::build()` packs each level in unit space (largest radius normalised to 1) via a golden-angle-spiral seed plus pairwise-separation/centroid-pull relaxation, so its convergence threshold stays meaningful regardless of the underlying byte scale; results are cached per `DirSizeNode` (nested packs) and per (focus, aspect) (the top-level pack) in a `BubbleLayout::PackCache` owned by `BubbleMapView`, so a resize only redoes the cheap affine fit-to-area — not the O(n² · iterations) packing — **when every pack it needs is already cached**. On a cache miss (a fresh scan, an unseen drill, or a resize into a new aspect bucket), `BubbleLayout::packsCached()` (walks the same top-level/group selection `build()` does, but only peeks the cache via `PackCache::tryTopPack()`/`tryNestedPack()`) tells `rebuildLayout()` to defer instead of packing on the GUI thread: it dispatches a `QtConcurrent::run()` job onto a dedicated single-thread `QThreadPool` (`bubblePackThreadPool()` in bubble_map_view.cpp — deliberately not the global pool `DirSizeScanner` uses, so a cold pack can never compete with a running scan) via `QFutureWatcher<BubbleLayout::PackCache>`, and on landing merges the result into `mPackCache` (`PackCache::mergeFrom()`, GUI-thread only) before re-deriving against the *live* focus/area and re-checking `packsCached()` — never assuming a landed result is automatically enough, since a drill or another resize can have arrived while it was packing. A `mTreeGeneration` counter (bumped only in `rootAboutToChange()`, before `mPackCache.clear()`) guards the merge against a tree replaced mid-pack, whose stale `DirSizeNode*` keys must never be looked up (or worse, silently collide with a *new* tree's node addresses via heap reuse) again; a separate `mPackWorkerTrackedSeq`/`BubbleLayout::PackKey` pair coalesces repeated requests for the same (focus, aspect-bucket) — e.g. a resize drag re-entering the same bucket — onto whichever single dispatch is already in flight, instead of fanning out a worker per request. This deliberately does **not** follow the house `QPointer` + `invokeMethod` + destructor-`waitForFinished()` worker-lifetime pattern used everywhere else in this document (Maintenance Wizard, menu-bar/tray monitors, disk-health/process collection): the packing task captures no `this` at all, only value copies (a `DirSizeNodePtr` keeping the tree alive, a `PackCache` snapshot, a raw `DirSizeNode*` kept valid by that same shared_ptr copy), so there is no view state for a `QPointer` to guard and nothing to marshal back except the value the `QFuture` itself returns; the `QFutureWatcher` is simply an ordinary child `QObject` of the view, so if the view is destroyed first Qt disconnects and drops the eventual `finished()` delivery on its own — there is no backstop `waitForFinished()` to write because there is nothing left to wait for that could touch a dead view. Do not "fix" this back to the house pattern; it would reintroduce a `this` capture this design specifically avoids. `mLayout`'s raw `DirSizeNode*`s (and the per-node hue assignment used to colour them, from `DiskMapView::mHueSlots`) are deliberately left showing the *previous* request's still-valid content while a newer one packs, so the view is never blank — `mDisplayedTree` (a second `DirSizeNodePtr`, independent of `mRoot`) and `mDisplayedHueSlots` (a frozen copy of the hue map, via `DiskMapView::hueSlotsSnapshot()`) exist purely so that stale content stays both alive and correctly coloured (rather than falling back to a default hue) until the new layout actually lands. |
| `shared/nexis/Pages/DiskMap/sunburst_view.h/.cpp`, `sunburst_layout.h/.cpp` | SSO-23862, two-level redesign in SSO-24963: sunburst (radial) mode. `SunburstLayout::build()` lays ring 0 out as wedges swept proportional to size, then subdivides each ring-0 directory's arc into ring-1 wedges for its own children; a ring-0 file or empty directory gets a single desaturated ring-1 placeholder wedge. Siblings whose wedge would be imperceptibly thin are folded into a single trailing "remainder" wedge per parent span, but at least `Metrics::minKeptWedges` (12) of the largest are always kept as real wedges so a folder with many similar-sized children stays navigable instead of collapsing into one blank remainder. Drilling in either mode plays the shared `DiskMapView::armCrossFade()`/`startCrossFadeIfArmed()`/`paintCrossFadeOverlay()` cross-fade (TreemapView instead plays its own geometric zoom). |
| `shared/nexis/Pages/Resources/disk_treemap_dialog.h/.cpp` | SSO-3737 / FW-09, extended in SSO-23862: wraps `DirSizeScanner` + all three `DiskMapView` modes behind a `QStackedWidget` and a toolbar picker. Every scan/drill/theme change is applied to all three views in lockstep (not just the visible one) so switching modes is instant and never re-scans or re-derives from a different tree snapshot. Routes "Move to trash" through `FileSearchService` to share the cleaner's trash path — a reversible action, so it keeps the lightweight `QMessageBox::question` confirm used elsewhere for trash (e.g. `DiskToolsPage::onLargeOldTrash`), not the File Shredder's heavier preview/irreversibility-warning dialog (SSO-15381), which exists because shredding can't be undone. SSO-23861: the dialog listens for `FileSearchService::fileOperationFinished` and re-scans `mLastScannedPath` on a successful trash so all three views reflect the removal. `test-DiskTreemapDeleteConfirmTests` drives the real confirm dialog under `QT_QPA_PLATFORM=offscreen` (no interactive desktop available in CI) via `QMetaObject::invokeMethod` on `onTrashRequested` — Cancel leaves the file in place with no `FileSearchService` call, Confirm runs a real `moveToTrash()` and asserts the map's file count drops after the re-scan. |
| `shared/nexis/Common/trust_safety_runner.h/.cpp`, `trust_safety_preview_dialog.h/.cpp`, `trust_safety_types.h` | SSO-15380: shared explain-before-run / itemized-preview / cancel / dry-run component for maintenance surfaces. `TrustSafetyRunner` follows the `DirSizeScanner` off-thread + pollable-cancel-flag pattern; `TrustSafetyPreviewDialog` takes any `TrustSafetyActionProvider`. GH#441: `TrustSafetyActionProvider` gained an optional `beginExecution(items, dryRun)` hook, called once by `TrustSafetyRunner::executeSynchronous()` with the full selected batch before the per-item `performItem()` loop starts (skipped for a dry run); it defaults to a no-op, so only providers that need it override it. See `shared/nexis/Common/README.md` for adoption. |
