...
**Goal:** Enable Ubuntu/Debian users to install Nexis via `sudo add-apt-repository ppa:s4solutionsllc/nexis && sudo apt install nexis`, with automated PPA publishing on every tagged release.
...
9. Verify the PPA exists: `https://launchpad.net/~s4solutionsllc/+archive/ubuntu/nexis`
  ...
  publish-ppa:
    ...
    steps:
      ...
      - name: Publish to PPA
        ...
        with:
          repository: ppa:s4solutionsllc/nexis
...
```markdown
- **PPA repository** -- Ubuntu users can now install via `sudo add-apt-repository ppa:s4solutionsllc/nexis && sudo apt install nexis` with automatic updates. Supports Ubuntu 22.04+, x86_64 and ARM64 (FR-90)
```
...
```markdown
- **PPA (Ubuntu)** -- `ppa:s4solutionsllc/nexis` for Ubuntu 22.04+, automatic updates via apt
```
...
4. Monitor Launchpad: check `https://launchpad.net/~s4solutionsllc/+archive/ubuntu/nexis/+packages` for pending builds
