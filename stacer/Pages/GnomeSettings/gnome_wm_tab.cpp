    ...
    mLoading(false)
{
    ...
    connect(ui->chkAutoRaise, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::WM_PREFS, GnomeKey::AUTO_RAISE, checked);
    });
    connect(ui->chkRaiseOnClick, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::WM_PREFS, GnomeKey::RAISE_ON_CLICK, checked);
    });

    // Mutter checkboxes
    connect(ui->chkDynamicWorkspaces, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::MUTTER, GnomeKey::DYNAMIC_WORKSPACES, checked);
    });
    connect(ui->chkEdgeTiling, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::MUTTER, GnomeKey::EDGE_TILING, checked);
    });
    connect(ui->chkAutoMaximize, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::MUTTER, GnomeKey::AUTO_MAXIMIZE, checked);
    });
    connect(ui->chkCenterNewWindows, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::MUTTER, GnomeKey::CENTER_NEW_WINDOWS, checked);
    });
    connect(ui->chkWorkspacesPrimary, &QCheckBox::toggled, this, [this](bool checked) {
        if (mLoading) return;
        GnomeSettingsTool::setB(GnomeSchema::MUTTER, GnomeKey::WORKSPACES_PRIMARY, checked);
    });
}
