...
void NetworkDiagWidget::buildUI()
{
    ...
    mLblLoading->setObjectName("netDiagSecondary");
...
void NetworkDiagWidget::refreshThemeColors()
{
    ...
    for (QFrame *card : findChildren<QFrame *>("netDiagCard"))
        card->setStyleSheet(cardStyle);

    if (mLblDnsHeader)
        mLblDnsHeader->setStyleSheet(
            QString("color: %1; font-size: 14px; font-weight: bold;").arg(textPri));

    if (mLblInterface)
        mLblInterface->setStyleSheet(
            QString("color: %1; font-size: 12px;").arg(textSec));

    if (mLblLoading)
        mLblLoading->setStyleSheet(
            QString("color: %1; font-size: 13px;").arg(textSec));

    if (mBtnRetest) {
        mBtnRetest->setStyleSheet(QString(
            "QPushButton#netDiagRetest {"
            "  background-color: %1;"
            "  color: %4;"
            "  border: none;"
            "  border-radius: 4px;"
            ...
            "  font-size: 12px;"
            "}"
            "QPushButton#netDiagRetest:hover {"
            "  opacity: 0.9;"
            "}"
            "QPushButton#netDiagRetest:disabled {"
            "  background-color: %2;"
            "  color: %3;"
            "}").arg(accent, cardBg, textSec, textBtn));
    }
}
